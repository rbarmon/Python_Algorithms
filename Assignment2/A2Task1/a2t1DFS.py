...
def deBFS(graph, visited, source, target, bottleneck):
    if source == target:
        return bottleneck
...
def notworkingDFS(graph, visited, u, t, bottleneck):
    if u == t:
        return bottleneck
    # d3:      if s = t then return bottleneck // We hit the sink, so we have an augmenting path
    ...
    # d7:          if residual > 0 and not visited[v] then
            ...
            augment = BFS(graph, visited, u_adj_edge.index, t, min(bottleneck, residual))
    # d8:              augment = DFS(v , t , min(bottleneck, residual))
            if augment > 0:
    # d9:              if augment > 0 then // We found an augmenting path - add the flow
                u_adj_edge.reversible_backward_edge += augment
...
def BFS(graph, visited, u, t, bottleneck):
    if u == t:
        return bottleneck
    # d3:      if s = t then return bottleneck // We hit the sink, so we have an augmenting path
