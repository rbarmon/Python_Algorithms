...
def optimalRoute(start, end, passengers, roads):
    """ NOT DONE
    	...
    	Approach description: The main constraints in this problem was to accomodate for the changing edge weights after
    	a passenger has rode the car and how to find the shortest path from start to end when there are additional
    	...
    	up a passenger. There are primarily two cases for this where:
    	    ...
    	    ever in the car it would run like regular Dijkstra.
    	    2. if the relaxed node is a node that is accessible with a passenger in the car then the node's values are updated
    	    in a new dist and pred array with the distances and pred information from when a passenger can first be picked up.
    	    This second case functions as another frame of dijkstra in the sense that at most Dijkstra's algorithm runs for
    	    twice to pop all the values in the priority queue.
    	Once all values are popped from the priority queue all that remains is to traverse the pred lists for both cases
    	if necessary and return the final path.
...
class Min_Heap_Priority_Queue:
    ...
    def push(self, x):
        ...
        Function description: Function to Insert an element into the heap. This is the Insert function Algorithm 11 from
         the same section in the course.
    ...
    def min_rise(self, i):
        """NOT DONE
        Function description: Function to move the element up the heap until it satisfies the heap propperty for min-heap.
        Based on Algorithm 13 Heap: rise
        Input:
    ...
    def swap(self, i, j):
        """NOT DONE
        Function description: Function to swap the position between two indices in the array.

        Input:
        	x: index of first position
        	j: index of second position
        """
    ...
    def sink(self, i):
        """NOT DONE
        Function description: This moves an element down the heap until it satifies the heap property of minheap. Based
        on Algorithm 14 Heap:Fall.
    ...
    def smallest_child(self, i):
        """NOT DONE
        Function description: Function to find the index of i's child with smallest value. Based on 1008 Heap DS largest_child.
    ...
    # print(passenger_condition)
            ...
            while temp:  # halts on None for adj node
                ...
                # if new_dist[temp.vertexNo] > new_dist[u.index] + temp.double: cheap fix
                    ...
                    # pred[v] = u
                    Queue.push(MinHeapNode(temp.vertexNo, new_dist[temp.vertexNo], temp.vertexNo in passengers or u.is_passenger_node))
                    # temp = temp.next
                    # Queue.printPQ()
                    # Q.push(v, key=dist[v])
